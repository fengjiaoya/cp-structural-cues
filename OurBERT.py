...
from sklearn.metrics import f1_score, accuracy_score, precision_recall_curve, average_precision_score, precision_recall_fscore_support
...
# ✅ Add this block immediately below
    ...
    m_f1_test = f1_score(y_test, pred_top1, average="macro")

  
    from sklearn.metrics import precision_recall_fscore_support, accuracy_score

    prec_macro_test, rec_macro_test, _, _ = precision_recall_fscore_support(
        y_test, pred_top1, average="macro", zero_division=0
    )
    prec_w_test, rec_w_test, f1_w_test, _ = precision_recall_fscore_support(
        y_test, pred_top1, average="weighted", zero_division=0
    )

    acc_test = accuracy_score(y_test, pred_top1)

    acc_weight_test = rec_w_test



    print(
    "\n[TEST] "
    f"Macro AUPRC: {m_auprc_test:.6f} | "
    f"Recall@P>={args.p_thresh:.2f} (macro): {r_pX_macro_test:.6f} | "
    f"Macro-F1: {m_f1_test:.6f} | "
    f"Precision(macro): {prec_macro_test:.6f} | "
    f"Recall(macro): {rec_macro_test:.6f} | "
    f"Precision(weighted): {prec_w_test:.6f} | "
    f"Recall(weighted): {rec_w_test:.6f} | "
    f"F1(weighted): {f1_w_test:.6f} | "
    f"Accuracy: {acc_test:.6f} | "
    f"Accuracy_weight: {acc_weight_test:.6f}"
)
    ...
    y_bin_test = label_binarize(y_test, classes=np.arange(num_labels))
        ...
        # 1) Per-class PR curve points (precision–recall pairs across thresholds)
    with open(os.path.join(metrics_dir, "per_class_pr_curves_test.csv"), "w", newline="", encoding="utf-8") as f:
        ...
        # 2) Per-class PRF on argmax predictions
    with open(os.path.join(metrics_dir, "per_class_prf_test.csv"), "w", newline="", encoding="utf-8") as f:
            import csv
            w = csv.writer(f)
            w.writerow(["class_id", "class_name", "precision", "recall", "f1"])
            for c in range(num_labels):
    ...
    with open(os.path.join(metrics_dir, "summary_all_metrics_test.csv"), "w", newline="", encoding="utf-8") as f:
        import csv
        w = csv.writer(f)
        w.writerow([
            "macro_auprc", f"recall_at_p{int(args.p_thresh*100)}_macro",
            "macro_f1", "precision_macro", "recall_macro",
            "precision_weighted", "recall_weighted", "f1_weighted",
            "accuracy", "accuracy_weight"
        ])
        w.writerow([
            m_auprc_test, r_pX_macro_test, m_f1_test,
            prec_macro_test, rec_macro_test,
            prec_w_test, rec_w_test, f1_w_test,
            acc_test, acc_weight_test
        ])
    ...
    summary = {
        ...
        "test_precision_macro": float(prec_macro_test),
        "test_recall_macro": float(rec_macro_test),
        "test_precision_weighted": float(prec_w_test),
        "test_recall_weighted": float(rec_w_test),   # == test_accuracy
        "test_f1_weighted": float(f1_w_test),
        "test_accuracy": float(acc_test),
        "test_accuracy_weight": float(acc_weight_test)  # 选项 A：= recall_weighted
    }
